...
import Config._
import TcpDsBenchmark.appendToCsv
...
object tpcdsbenchmark1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Values partitioning on big tables, partitioning thinking in one nodes.

    // Values partitioning on big tables, partitioning thinking in few nodes.

    //TODO: use this configuration with partition 1 (cores / cores)
    ...
    //Values for partition 1 GB thinking in a distributed system
...
//    val catalogSalesTable = spark.read.jdbc(URL, CATALOG_SALES, CATALOG_SALES_PARTITIONING_KEY, 0, 15000, parallelismLevel , mysqlConnProperties)
//
//    //val customerTable = spark.read.jdbc(URL, CUSTOMER, CUSTOMER_PARTITIONING_KEY, 0, 270000, parallelismLevel / 2,mysqlConnProperties)
//    val customerTable = spark.read.jdbc(URL, CUSTOMER, mysqlConnProperties)
//
//    //val dateTimTable = spark.read.jdbc(URL, DATE_DIM, DATE_DIM_PARTITIONING_KEY,  0, 70000, parallelismLevel , mysqlConnProperties)
//    val dateTimTable = spark.read.jdbc(URL, DATE_DIM, DATE_DIM_PARTITIONING_KEY,  0, 2300000, parallelismLevel / 2 , mysqlConnProperties)
//
//    val customAddressTable = spark.read.jdbc(URL, CUSTOMER_ADDRESS, mysqlConnProperties)
//
//    val customerDemographicsTable = spark.read.jdbc(URL, CUSTOMER_DEMOGRAPHICS, CUSTOMER_DEMOGRAPHICS_PARTITIONING_KEY, 0, 1800000 , parallelismLevel, mysqlConnProperties)
...
//    val timeDimTable = spark.read.jdbc(URL, TIME_DIM, mysqlConnProperties)
//    val webPageTable = spark.read.jdbc(URL, WEB_PAGE, mysqlConnProperties)
//    val webReturnsTable = spark.read.jdbc(URL, WEB_RETURNS, WEB_RETURNS_PARTITIONING_KEY,  0, 15000, parallelismLevel / 2 , mysqlConnProperties)
//    val warehouseTable = spark.read.jdbc(URL, WAREHOUSE, mysqlConnProperties)
...
//    val catalogPageTable = spark.read.jdbc(URL, CATALOG_PAGE, mysqlConnProperties)
//    val catalogReturnsTable = spark.read.jdbc(URL, CATALOG_RETURNS, mysqlConnProperties)
//    val catalogSalesTable = spark.read.jdbc(URL, CATALOG_SALES, CATALOG_SALES_PARTITIONING_KEY, 0, 50000, parallelismLevel , mysqlConnProperties)
//
//    //val customerTable = spark.read.jdbc(URL, CUSTOMER, CUSTOMER_PARTITIONING_KEY, 0, 270000, parallelismLevel / 2,mysqlConnProperties)
//    val customerTable = spark.read.jdbc(URL, CUSTOMER, mysqlConnProperties)
//
//    //val dateTimTable = spark.read.jdbc(URL, DATE_DIM, DATE_DIM_PARTITIONING_KEY,  0, 70000, parallelismLevel , mysqlConnProperties)
//    val dateTimTable = spark.read.jdbc(URL, DATE_DIM, DATE_DIM_PARTITIONING_KEY,  0, 70000, parallelismLevel / 2 , mysqlConnProperties)
//
//    val customAddressTable = spark.read.jdbc(URL, CUSTOMER_ADDRESS, mysqlConnProperties)
//
//    val customerDemographicsTable = spark.read.jdbc(URL, CUSTOMER_DEMOGRAPHICS, CUSTOMER_DEMOGRAPHICS_PARTITIONING_KEY, 0, 1800000 , parallelismLevel, mysqlConnProperties)
...
//    val houseHoldDemographicsTable = spark.read.jdbc(URL, HOUSEHOLD_DEMOGRAPHICS, mysqlConnProperties)
//    val incomeBandTable = spark.read.jdbc(URL, INCOME_BAND, mysqlConnProperties)
//    val inventoryTable = spark.read.jdbc(URL, INVENTORY,INVENTORY_PARTITIONING_KEY, 0, 50000, parallelismLevel, mysqlConnProperties)
//    val itemTable = spark.read.jdbc(URL, ITEM, mysqlConnProperties)
//    val promotionTable = spark.read.jdbc(URL, PROMOTION, mysqlConnProperties)
//    val reasonsTable = spark.read.jdbc(URL, REASON, mysqlConnProperties)
//    val shipModeTable = spark.read.jdbc(URL, SHIP_MODE, mysqlConnProperties)
//    val storeTable = spark.read.jdbc(URL, STORE, mysqlConnProperties)
//    val storeReturnsTable = spark.read.jdbc(URL, STORE_RETURNS, STORE_RETURNS_PARTITIONING_KEY, 0, 50000, parallelismLevel, mysqlConnProperties)
//    val storesSalesTable = spark.read.jdbc(URL, STORE_SALES, STORES_SALES_PARTITIONING_KEY,  0, 50000, parallelismLevel , mysqlConnProperties)
//    //val timeDimTable = spark.read.jdbc(URL, TIME_DIM, TIME_PARTITIONING_KEY,  0, 85000, parallelismLevel / 2 , mysqlConnProperties)
//    val timeDimTable = spark.read.jdbc(URL, TIME_DIM, mysqlConnProperties)
//    val webPageTable = spark.read.jdbc(URL, WEB_PAGE, mysqlConnProperties)
//    val webReturnsTable = spark.read.jdbc(URL, WEB_RETURNS, WEB_RETURNS_PARTITIONING_KEY,  0, 50000, parallelismLevel / 2 , mysqlConnProperties)
//    val warehouseTable = spark.read.jdbc(URL, WAREHOUSE, mysqlConnProperties)
//    val webSalesTable = spark.read.jdbc(URL, WEB_SALES, WEB_SALES_PARTITIONING_KEY, 0, 50000, parallelismLevel, mysqlConnProperties)
//    val webSiteTable = spark.read.jdbc(URL, WEB_SITE, mysqlConnProperties)
